...
void main(List<String> arguments) {
  ...
  for (final node in nodes) {
    if (node is LogicUnitNode) {
      print(' - LogicUnit: ${node.name} (state: ${node.stateVariables}, methods: ${node.methods})');
  ...
  if (mode == 'safe') {
    ...
    for (final entry in nodesByFile.entries) {
      ...
      final fileNodes = uniqueFileNodes.values.toList()..sort((a, b) => b.offset.compareTo(a.offset));
      
      for (final node in fileNodes) {
        String comment = '';
        if (node is LogicUnitNode) comment = '// TODO(Migrator): Convert ${node.name} to StateNotifier\n';
        else if (node is ProviderDeclarationNode) comment = '// TODO(Migrator): Replace ${node.providerType} with StateNotifierProvider\n';
        else if (node is ConsumerNode) comment = '// TODO(Migrator): Change to ConsumerWidget and use ref.watch\n';
        else if (node is ProviderOfNode) comment = '// TODO(Migrator): Replace with ref.read or ref.watch\n';
        else if (node is SelectorNode) comment = '// TODO(Migrator): Replace Selector with ref.watch(${node.consumedClass.toLowerCase()}Provider.select(...))\n';
        else if (node is MultiProviderNode) comment = '// TODO(Migrator): Remove MultiProvider, Riverpod providers are global. Wrap app in ProviderScope.\n';
        else if (node is AsyncProviderNode) comment = '// TODO(Migrator): ${node.providerType} exposes AsyncValue. Use .when() to handle data/loading/error states.\n';
    ...
    for (final entry in nodesByFile.entries) {
      ...
      buffer.writeln('// 🚀 Auto-generated Riverpod migration suggestions for ${originalFile.uri.pathSegments.last}');
      buffer.writeln('// You can use these snippets to replace your Provider logic.');
      buffer.writeln();
      
      final processedTypes = <String>{};
      for (final node in entry.value) {
        if (node is ProviderOfNode) {
          if (processedTypes.contains('ProviderOf:${node.consumedClass}')) continue;
          processedTypes.add('ProviderOf:${node.consumedClass}');
        } else if (node is ConsumerNode) {
          if (processedTypes.contains('Consumer:${node.consumedClass}')) continue;
