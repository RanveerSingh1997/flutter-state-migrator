...
Future<void> main(List<String> arguments) async {
  final parser = ArgParser()
    ...
    ..addFlag(
      ...
      negatable: false,
      help:
          'Scaffold migrator_config.yaml in the target project with annotated defaults',
    );
  ...
  if (argResults['init'] as bool) {
    final initTarget =
        argResults.rest.isNotEmpty ? argResults.rest.first : Directory.current.path;
    _runInit(initTarget);
    exit(0);
  }
...
void _runInit(String projectPath) {
  final configFile = File('$projectPath/migrator_config.yaml');
  if (!Directory(projectPath).existsSync()) {
    stderr.writeln('Error: path not found: $projectPath');
    exitCode = 64;
    return;
  }

  if (configFile.existsSync()) {
    print('⚠️  migrator_config.yaml already exists at $projectPath');
    print('   Delete it first if you want to regenerate defaults.');
    return;
  }

  const config = '''
# migrator_config.yaml — generated by `migrator --init`
# Place this file at your Flutter project root.
# All keys are optional; safe defaults apply when absent.

# Naming convention for generated Riverpod providers.
# Options: 'camelCase' (default) | 'snake_case'
provider_naming: camelCase

# When true, a ChangeNotifier with multiple state fields is collapsed into a
# single immutable state class (Freezed-compatible).
auto_merge_state: true

# Governance rules evaluated by GovernanceEngine.
# Remove or leave empty to disable governance checks.
governance:
  # Layer names that must not import from each other (colon-separated).
  # Example: "data:presentation" forbids data → presentation imports.
  # forbidden_layer_deps: []

  # Maximum number of direct dependencies allowed per component.
  max_coupling: 7

  # Maximum allowed depth of the dependency graph.
  max_depth: 5

# Override the inferred architecture role for class-name suffixes.
# Example: classes ending in 'Service' will be treated as 'repository'.
architecture_roles:
  # Service: repository
  # Manager: logic
''';
  ...
  print('✅ Created migrator_config.yaml at $projectPath');
  print('   Edit the file to customize governance rules and naming conventions.');
}
