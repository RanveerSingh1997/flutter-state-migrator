...
class RiverpodTransformer {
  ...
  List<TextEdit> _transformAsyncProvider(
    ...
    final isFuture = node.providerType == 'FutureProvider';
    final asyncKw = isFuture ? 'async' : 'async*';
    // Emit a compilable stub — `throw UnimplementedError()` is valid inside an
    // async function and makes the gap obvious without producing a syntax error.
    final stubBody = isFuture
        ? '// TODO(Migrator): implement — return the Future value\n  throw UnimplementedError();'
        : '// TODO(Migrator): implement — yield stream values\n  throw UnimplementedError();';

    final globalProviderDef =
        '''

// TODO(Migrator): Migrated from ${node.providerType}<${node.providedType}>
final $providerName = ${isFuture ? 'FutureProvider' : 'StreamProvider'}<${node.providedType}>((ref) $asyncKw {
  $stubBody
});
''';
...
''';
  ...
  List<TextEdit> _transformProxyProvider(
    ...
    // Append a skeleton Riverpod notifier. The `update:` lambda becomes a
    // ref.watch() call in build(). Returning a default-constructed instance
    // keeps the file compilable; the developer replaces it with real logic.
    final skeleton = '''

// TODO(Migrator): Migrated from $label<${node.baseType}, ${node.resultType}>
// Replace the build() body: wire up ${_lcFirst(node.baseType)} to produce ${node.resultType}.
@riverpod
class ${node.resultType} extends _\$${node.resultType} {
  ...
  ${node.resultType} build() {
    ...
    // TODO(Migrator): return ${node.resultType} using ${_lcFirst(node.baseType)}
    return ${node.resultType}();
  }
}
''';
    // resultProviderName is generated by @riverpod — no manual declaration needed.
