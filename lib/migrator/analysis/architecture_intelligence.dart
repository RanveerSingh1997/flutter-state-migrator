...
class ArchitectureIntelligenceEngine {
  ...
  List<ArchitectureSmell> _analyzeLogicUnit(String id, LogicUnitNode node) {
    ...
    if (node.methods.length > 15) {
      unitSmells.add(ArchitectureSmell(
        nodeId: id,
        name: 'God Component',
        description: 'Class ${node.name} has ${node.methods.length} methods. Consider splitting it into smaller, more focused units.',
        severity: 'warning',
      ));
    }
    ...
    if (node.stateFields.length > 10) {
      unitSmells.add(ArchitectureSmell(
        ...
        description: 'Class ${node.name} manages ${node.stateFields.length} state fields. This may lead to maintenance complexity and unnecessary rebuilds.',
        severity: 'warning',
      ));
    }
    ...
    if (dependencies.length > 7) {
      unitSmells.add(ArchitectureSmell(
        nodeId: id,
        name: 'High Coupling',
        description: 'Class ${node.name} depends on ${dependencies.length} other components. High coupling makes testing and refactoring difficult.',
        severity: 'warning',
      ));
    }
    ...
    if (asyncMethods > 3 && node.notifierType != NotifierType.asyncNotifier && node.isNotifier) {
      unitSmells.add(ArchitectureSmell(
        nodeId: id,
        name: 'Improper Async Pattern',
        description: 'Class ${node.name} has several async methods but isn\'t an AsyncNotifier. Consider using Riverpod\'s AsyncNotifier for better lifecycle safety.',
        severity: 'info',
      ));
    }
  ...
  List<ArchitectureSmell> _analyzeGraphMetrics() {
    ...
    for (final cycle in cycles) {
      graphSmells.add(ArchitectureSmell(
        nodeId: cycle.first,
