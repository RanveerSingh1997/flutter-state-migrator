...
void main() {
  group('AIManager', () {
    ...
    test(
      'falls back deterministically when local LLM is unavailable',
      () async {
        final manager = AIManager(
          client: MockClient(
    ...
    test(
      'builds architecture guidance from smells and governance violations',
      () async {
        ...
        final manager = AIManager(
          client: MockClient((_) async => http.Response('not ready', 500)),
        );
        ...
        final guidance = await manager.buildArchitectureGuidance(
          ...
          smells: [
            ArchitectureSmell(
              ...
              name: 'High Coupling',
              description:
                  'Class DashboardController depends on 9 other components.',
            ),
          ],
          violations: [
            GovernanceViolation(
              ...
              message:
                  'Architecture violation: Layer "presentation" is not allowed to depend on "data".',
            ),
          ],
        );
        ...
        expect(
          ...
          contains('Reduce direct dependencies'),
        );
